...
import com.conan.chatroom.service.ChatService;
...
public class ChatServiceImpl implements ChatService {
    ...
    public void singleSend(JSONObject param, ChannelHandlerContext ctx) {
        ...
        if (toUserCtx == null) {
            ...
            //对方离线时保存离线消息
            List<SendMsgModel> offlineMsgs = Constant.offlineMsgMap.get(toUserId);

            if (offlineMsgs == null) {
                offlineMsgs = new ArrayList<SendMsgModel>();
            }

            SendMsgModel offlineMsg = new SendMsgModel();

            offlineMsg.setUserId(toUserId);
            offlineMsg.setFromUserId(fromUserId);
            offlineMsg.setType("SINGLE_SENDING");
            offlineMsg.setContent(content);

            offlineMsgs.add(offlineMsg);
            // 先缓存到静态的Map里面  以后改成存到数据库或者redis
            Constant.offlineMsgMap.put(toUserId, offlineMsgs);
